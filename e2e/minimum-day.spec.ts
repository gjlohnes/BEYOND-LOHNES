...
test('MINIMUM DAY enables and derives essentials from committed history offline', async ({ page, context }) => {
  await page.goto('/#/today');
  await page.getByRole('button', { name: 'START DAY' }).click();

  const minimumDay = page.locator('.card').filter({ has: page.getByRole('heading', { name: 'Minimum Day' }) });
