...
describe('application-owned backup and restore',()=>{
  ...
  it('rolls back the destructive transaction if restoration fails',async()=>{const day=await startDay('WORK');const document=await createBackupDocument();const duplicate={...document,payload:{...document.payload,beyondDays:[document.payload.beyondDays[0],document.payload.beyondDays[0]]}};await expect(replaceRestore(JSON.stringify(duplicate),{confirmed:true,safetyExportSucceeded:true})).rejects.toBeTruthy();expect((await db.beyondDays.get(day.id))?.id).toBe(day.id);expect(await db.beyondDays.count()).toBe(1);});
