...
export function TodayScreen() {
  ...
  async function finishDay() { if (!dayId) return; await endDay(dayId); setDayId(null); setRec(null); setDecisionLocked(false); setStatus('BEYOND Day ended.'); }
  async function checkIn(event: FormEvent<HTMLFormElement>) {
    ...
    try { const result = await submitCheckIn(dayId, { energy:number('energy') as 1|2|3|4|5, stress:number('stress') as 1|2|3|4|5, mood:number('mood') as 1|2|3|4|5, soreness:number('soreness') as 0|1|2|3|4|5, alcoholUrge:number('alcoholUrge') as 0|1|2|3|4|5 }); setRec(result.recommendation); setDecisionLocked(false); setStatus('REASSESS completed.'); }
    catch (error) { setStatus(error instanceof Error ? error.message : 'Check-in rejected.'); }
  }
  async function decide(decision:'ACCEPT'|'DISMISS'|'NO_ACTION') { if(!rec||decisionLocked)return; try { await decideRecommendation(rec.id,decision); setDecisionLocked(true); setStatus(`${decision} stored.`); if(decision==='ACCEPT'&&rec.suggestedCommand==='START_RESET') navigate(`/reset?recommendationId=${rec.id}`); else if(decision==='ACCEPT'&&rec.suggestedCommand==='START_SHIFT_DOWN'&&dayId){const result=await startShiftDown(dayId,rec.id);if(result.status==='COMPLETED')setShiftDownCommandId(result.commandId);} } catch(error){setStatus(error instanceof Error?error.message:'Decision not stored.');} }
  async function override(command:'START_RESET'|'START_SHIFT_DOWN'){if(!rec||decisionLocked)return;try{await decideRecommendation(rec.id,'OVERRIDE',command);setDecisionLocked(true);setStatus(`OVERRIDE stored. ${command} selected.`);}catch(error){setStatus(error instanceof Error?error.message:'Override not stored.');}}
  ...
  return <section><div className="eyebrow">BEYOND // TODAY</div><h1>Command</h1>{status&&<p role="status" className="card">{status}</p>}{!dayId?<div className="card"><h2>No active day</h2><p className="muted">A BEYOND Day starts when you start it.</p><button onClick={begin}>START DAY</button></div>:<>
    <div className="card"><h2>{rec?.title??'State check-in required'}</h2><p>{rec?.rationale??'Record current state to produce one deterministic recommendation.'}</p>{rec&&<><p><Link to={`/why/${rec.id}`}>WHY</Link></p>{rec.statusAtIssue==='NO_ACTION_REQUIRED'?<button disabled={decisionLocked} onClick={()=>decide('NO_ACTION')}>RECORD NO ACTION</button>:<><button disabled={decisionLocked} onClick={()=>decide('ACCEPT')}>ACCEPT</button>{' '}<button disabled={decisionLocked} onClick={()=>decide('DISMISS')}>DISMISS</button></>}<p className="muted">Override:</p><button disabled={decisionLocked} onClick={()=>override('START_RESET')}>RESET</button>{' '}<button disabled={decisionLocked} onClick={()=>override('START_SHIFT_DOWN')}>SHIFT DOWN</button></>}</div>
    ...
    <div className="card"><h2>Day lifecycle</h2><button onClick={finishDay}>END DAY</button><p className="muted">Explicit wake-to-sleep boundary. Never midnight rollover.</p></div></>}</section>;
}
