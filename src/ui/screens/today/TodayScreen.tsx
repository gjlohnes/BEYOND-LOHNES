...
import { endDay, getTodayState, startDay, submitCheckIn, type PersistedRecommendationDecision } from '../../../application/services/dayService';
...
export function TodayScreen() {
  ...
  async function decide(nextDecision:'ACCEPT'|'DISMISS'|'NO_ACTION') {
    ...
    try {
      ...
      if(nextDecision==='ACCEPT'&&rec.suggestedCommand==='START_RESET') navigate(`/reset?recommendationId=${rec.id}`);
      else if(nextDecision==='ACCEPT'&&rec.suggestedCommand==='START_SHIFT_DOWN'&&dayId){const result=await startShiftDown(dayId,rec.id);if(result.status==='COMPLETED')setShiftDownCommandId(result.commandId);}
    } catch(error) {
  ...
  async function override(command:'START_RESET'|'START_SHIFT_DOWN'){
    ...
    try{
      ...
      setStatus(`Recommendation overridden with ${command === 'START_RESET' ? 'RESET' : 'SHIFT DOWN'}.`);
    }catch(error){
      if (error instanceof Error && error.message === 'RECOMMENDATION_ALREADY_DECIDED') {
        await refreshTodayState();
        return;
  ...
  return <section><div className="eyebrow">BEYOND // TODAY</div><h1>Command</h1>{status&&<p role="status" className="card">{status}</p>}{!dayId?<div className="card"><h2>No active day</h2><p className="muted">A BEYOND Day starts when you start it.</p><button onClick={begin}>START DAY</button></div>:<>
    <div className="card"><h2>{rec?.title??'State check-in required'}</h2><p>{rec?.rationale??'Record current state to produce one deterministic recommendation.'}</p>{rec&&<><p><Link to={`/why/${rec.id}`}>WHY</Link></p>{decision?<p><strong>Decision: {decisionLabel(decision)}</strong></p>:<>{rec.statusAtIssue==='NO_ACTION_REQUIRED'?<button onClick={()=>decide('NO_ACTION')}>RECORD NO ACTION</button>:<><button onClick={()=>decide('ACCEPT')}>ACCEPT</button>{' '}<button onClick={()=>decide('DISMISS')}>DISMISS</button></>}<p className="muted">Override:</p><button onClick={()=>override('START_RESET')}>RESET</button>{' '}<button onClick={()=>override('START_SHIFT_DOWN')}>SHIFT DOWN</button></>}</>}</div>
