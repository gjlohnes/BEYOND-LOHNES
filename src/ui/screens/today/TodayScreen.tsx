...
export function TodayScreen() {
  ...
  async function checkIn(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setStatus('REASSESS completed. Check-in and recommendation stored.');
    } catch (error) {
  ...
  async function override(command: 'START_RESET' | 'START_SHIFT_DOWN') {
    ...
    try {
      ...
      setStatus(`OVERRIDE stored. ${command} selected.`);
    } catch (error) {
      setStatus(error instanceof Error ? error.message : 'Override not stored.');
    }
  }
  ...
  async function beginShiftDown() {
    ...
    if (result.status === 'COMPLETED') {
      ...
      setStatus(result.errorCode ?? 'SHIFT DOWN not started.');
    }
  }
