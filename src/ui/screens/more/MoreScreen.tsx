...
export function MoreScreen() {
  ...
  async function validateSelectedBackup() {
    ...
    try {
      ...
      setCandidate(null);
      setStatus(error instanceof Error ? `Backup rejected: ${error.message}` : 'Backup rejected.');
    }
  }
  ...
  async function restore() {
    ...
    } catch (error) { setStatus(error instanceof Error ? `Restore failed: ${error.message}` : 'Restore failed; current data was not intentionally cleared.'); }
  }
  ...
  return <section>
    ...
    <div className="card"><h2>Restore</h2>
      ...
      {selectedFile && <><p><strong>Selected:</strong> {selectedFile.name}</p>{!candidate && <button onClick={validateSelectedBackup}>VALIDATE BACKUP</button>}</>}
      {candidate && <div role="status"><p><strong>Backup valid.</strong></p><p>Days {candidate.preview.beyondDays} · Events {candidate.preview.events} · Recommendations {candidate.preview.recommendations} · Outcomes {candidate.preview.outcomes}</p><button onClick={restore}>REPLACE RESTORE</button></div>}
      {status && <p>{status}</p>}
    </div>
