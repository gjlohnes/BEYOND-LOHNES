...
export interface BackupPreview { beyondDays: number; events: number; recommendations: number; outcomes: number; }

function checkCompatibility(document: BackupDocument) {
  if (document.formatVersion !== BACKUP_FORMAT_VERSION) throw new Error('UNSUPPORTED_BACKUP_FORMAT_VERSION');
  if (document.dataSchemaVersion > DATA_SCHEMA_VERSION) throw new Error('UNSUPPORTED_FUTURE_DATA_SCHEMA_VERSION');
  if (document.dataSchemaVersion < DATA_SCHEMA_VERSION) throw new Error('BACKUP_MIGRATION_NOT_AVAILABLE');
}

export async function createBackupDocument(): Promise<BackupDocument> {
  const [meta, beyondDays, events, recommendations, outcomes] = await Promise.all([
    db.meta.toArray(), db.beyondDays.toArray(), db.events.toArray(), db.recommendations.toArray(), db.outcomes.toArray(),
  ]);
...
export async function serializeCurrentBackup() { return JSON.stringify(await createBackupDocument(), null, 2); }

export function parseBackup(raw: string): BackupDocument {
  let candidate: unknown;
  try { candidate = JSON.parse(raw); } catch { throw new Error('INVALID_BACKUP_JSON'); }
  const result = backupSchema.safeParse(candidate);
  if (!result.success) throw new Error('INVALID_BACKUP_DOCUMENT');
  checkCompatibility(result.data);
  return result.data;
}
...
export async function downloadBackup(): Promise<BackupDocument> {
  ...
  downloadText(`beyond-backup-${backup.exportedAt.replace(/[:.]/g, '-')}.json`, JSON.stringify(backup, null, 2));
...
export async function replaceRestore(raw: string, options: { confirmed: boolean; safetyExportSucceeded: boolean }) {
  ...
  await db.transaction('rw', db.meta, db.beyondDays, db.events, db.recommendations, db.outcomes, async () => {
    await Promise.all([db.meta.clear(), db.beyondDays.clear(), db.events.clear(), db.recommendations.clear(), db.outcomes.clear()]);
    ...
    await db.recommendations.bulkAdd(candidate.payload.recommendations.map(assertValidRecommendation));
