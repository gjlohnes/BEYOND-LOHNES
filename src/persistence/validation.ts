...
export const beyondDaySchema = z.object({
  ...
  if (day.status === 'COMPLETED' && !day.endedAt) ctx.addIssue({ code: 'custom', message: 'Completed day requires endedAt' });
  if (day.status === 'ACTIVE' && day.endedAt) ctx.addIssue({ code: 'custom', message: 'Active day cannot have endedAt' });
});
...
export const decisionTraceSchema = z.object({
  ...
  matchedRules: z.array(z.object({ ruleId: z.string().min(1), result: z.boolean(), reason: z.string() })),
...
export const recommendationSchema = z.object({
  ...
  suggestedCommand: z.enum(['START_RESET','START_SHIFT_DOWN','REASSESS','ENABLE_MINIMUM_DAY','START_WORKOUT','START_REDUCED_WORKOUT','RECOVERY_SESSION','LOG_WATER','PROTEIN_ACTION','SLEEP_PROTECTION','SUGGEST_ACTIVITY','START_DAY','END_DAY']).optional(),
...
export const domainEventSchema = z.object({
  ...
  if (!result.success) ctx.addIssue({ code: 'custom', message: `Invalid ${event.type} payload` });
});
