...
export async function startDay(workContext: WorkContext) {
  ...
  const event = assertValidEvent({
    id: crypto.randomUUID(), schemaVersion: 1, type: 'DAY_STARTED', beyondDayId: id,
    occurredAt: now, recordedAt: now, payload: { workContext }, source: 'USER',
  });
...
export async function endDay(dayId: string) {
  ...
  const completed = assertValidBeyondDay({ ...day, status: 'COMPLETED', endedAt: now, updatedAt: now });
  const started = assertValidEvent({
    id: crypto.randomUUID(), schemaVersion: 1, type: 'COMMAND_STARTED', beyondDayId: dayId,
    occurredAt: now, recordedAt: now, payload: { commandName: 'END_DAY' }, source: 'USER', correlationId: commandId,
  });
  const ended = assertValidEvent({
    id: crypto.randomUUID(), schemaVersion: 1, type: 'DAY_ENDED', beyondDayId: dayId,
    occurredAt: now, recordedAt: now, payload: { commandId }, source: 'USER', correlationId: commandId, causationId: started.id,
  });
  const commandCompleted = assertValidEvent({
    id: crypto.randomUUID(), schemaVersion: 1, type: 'COMMAND_COMPLETED', beyondDayId: dayId,
    occurredAt: now, recordedAt: now, payload: { commandName: 'END_DAY' }, source: 'SYSTEM', correlationId: commandId, causationId: started.id,
  });
  await db.transaction('rw', db.beyondDays, db.events, async () => {
...
export async function submitCheckIn(dayId: string, values: Omit<StateCheckIn, 'id' | 'beyondDayId' | 'recordedAt'>) {
  ...
  const now = new Date().toISOString();
  const checkIn = stateCheckInPayloadSchema.parse({ id: crypto.randomUUID(), beyondDayId: dayId, recordedAt: now, ...input }) as StateCheckIn;
  const reassessCommand = { id: crypto.randomUUID(), name: 'REASSESS' as const, beyondDayId: dayId, issuedAt: now, input };
  ...
  const recentEvents = await db.events.where('beyondDayId').equals(dayId).reverse().sortBy('occurredAt');
  const result = evaluate({ beyondDay: day, latestCheckIn: checkIn, recentEvents: recentEvents.slice(0, 25), context: {}, now });
  const recommendation = assertValidRecommendation({ id: crypto.randomUUID(), beyondDayId: dayId, issuedAt: now, ...result.primary, trace: result.trace });
  const checkInEvent = assertValidEvent({
    id: crypto.randomUUID(), schemaVersion: 1, type: 'STATE_CHECKED_IN', beyondDayId: dayId,
    occurredAt: now, recordedAt: now, payload: checkIn, source: 'USER', correlationId: reassessCommand.id,
  });
  const issued = assertValidEvent({
    id: crypto.randomUUID(), schemaVersion: 1, type: 'RECOMMENDATION_ISSUED', beyondDayId: dayId,
    occurredAt: now, recordedAt: now, payload: { recommendationId: recommendation.id, kind: recommendation.kind }, source: 'ENGINE', causationId: reassessCommand.id,
  });
